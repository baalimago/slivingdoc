...
budget. Without the pre-build, the gate can only pass on a machine whose
...
The timeout applies to the whole three-count run of one package. A suite
...
other than its own. Per-package coverage understates it badly.

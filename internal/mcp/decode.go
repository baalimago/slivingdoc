...
// path of at most 4,096 bytes without U+0000. Unknown fields, duplicate
// fields, explicit null, non-object arguments, and malformed JSON are
...
// validatePath applies the path contract: an absolute UTF-8 host path with
// 1 through 4,096 bytes, without U+0000. The path must stay below the
// configured workspace root; the service enforces that rule when it
